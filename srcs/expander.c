...
int	expander(t_command *command, char **envp)
{
	int	i;
	char	*change;
	...
	{
		i = 0;
		while (command->full_cmd_args[i])
		{
			change = check_env(command->full_cmd_args[i], envp);
			if (change)
			{
				free(command->full_cmd_args[i]);
				command->full_cmd_args[i] = change;
			}
			i++;
		}
